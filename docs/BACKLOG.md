...
| ~~6b~~ | ~~Remove the standalone `CorsFilter` from work-service~~ | **Done at M2.** `SecurityFilterChain` now installs CORS from the same `CorsConfigurationSource` bean, and the temporary `FilterRegistrationBean` is gone | — |
