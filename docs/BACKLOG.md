...
| 6b | The standalone `CorsFilter` registration in `CorsConfig` must be **removed** at M2 | plain Spring MVC ignores a `CorsConfigurationSource` bean, so a filter is needed today. At M2 the `SecurityFilterChain` installs its own from that same bean; leaving both writes `Access-Control-Allow-Origin` twice and the browser rejects a duplicated header | M2 |
| 11 | No client-side routing — the frontend switches screens by state, the URL never changes | two screens do not justify a router dependency; you cannot refresh into a board or share a link to one | when a third screen arrives |
